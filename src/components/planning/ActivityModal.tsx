...
interface ActivityModalProps {
  isOpen: boolean;
  activityToEdit?: PlannerActivity | null;
  defaultDate?: string;
  onClose: () => void;
  onSave: (data: Omit<PlannerActivity, 'id' | 'createdAt'>) => void;
...
export const ActivityModal: React.FC<ActivityModalProps> = ({
  ...
  const [category, setCategory] = useState<'Estudos' | 'Trabalho' | 'Pessoal' | 'Saúde' | 'Outros'>('Estudos');
  ...
  useEffect(() => {
    if (activityToEdit) {
      ...
      setDateString(activityToEdit.dateString);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      isCompleted: activityToEdit ? activityToEdit.isCompleted : false,
      category,
      priority,
    });
    onClose();
      ...
      <div className="w-full max-w-lg my-auto p-6 rounded-3xl bg-zinc-900 border border-zinc-800 shadow-2xl text-white space-y-6 relative">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-bold tracking-tight text-white">
            {activityToEdit ? 'Editar Atividade' : 'Nova Atividade do Planejamento'}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-zinc-300 block">Título da Atividade</label>
            <input
              type="text"
              required
              placeholder="Ex: Estudar Next.js, Fazer Exercícios..."
              value={title}
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-zinc-300 block">Descrição / Notas</label>
            <textarea
              rows={2}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-zinc-300 block">Horário Inicial</label>
              <input
                type="time"
                required
                value={startTime}
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-zinc-300 block">Duração (Minutos)</label>
              <input
                type="number"
                min={5}
                max={480}
